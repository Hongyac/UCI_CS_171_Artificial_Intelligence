...
#             print('X, Y, Z, p :', X, Y, Z, p)
            if Z == None:
                printV('i:', i, 'j:', j, 'k:', k, '', X, '->', Y,'['+str(p)+']',
                       'PY =' ,getP(Y, k, k), p, '=', getP(Y, k, k) * p)
                PY = getP(Y, k, k) * p
                if PY > getP(X, k, k):
                    printV('     inserting from', k, '-', k, ' ', X, '->', T[Y+'/'+str(k)+'/'+str(k)],
                                'because', PY, '=', getP(Y, i, j), '*', p, '>', getP(X, i, j), '=',
                                'getP(' + X + ',' + str(i) + ',' + str(j) + ')')
                    P[X + '/' + str(i) + '/' + str(k)] = PY
                    T[X + '/' + str(i) + '/' + str(k)] = Tree.Tree(X,T[Y+'/'+str(k)+'/'+str(k)], None)
                    printV('P:', P)
                    printV('T:', [str(t)+':'+str(T[t]) for t in T])
            else:
...
#     CYKParse(['I', 'saw', 'a', 'man', 'with', 'my', 'telescope'], getGrammarTelescope())
    CYKParse(['hi', 'I', 'am'], getGrammarWeather())
...
#     CYKParse(['what', 'is', 'now', 'the', 'temperature', 'in', 'Irvine'], getGrammarWeather())
    ...
    CYKParse(['Will', 'yesterday', 'be', 'hotter', 'than', 'tomorrow', 'in', 'Pasadena'], getGrammarWeather())
